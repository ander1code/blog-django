...
def do_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render ( request , 'login.html', {'statuslog':1})
        else:
            return render ( request , 'login.html', {'statuslog':0})
    else:
...
def criar(request):
    if request.method == 'POST':
        ...
        frm = frmBlog (initial={'autor': request.user.id, 'dataCriacao':dataCriacao, 'titulo': '' , 'briefing': '', 'texto': ''} )
    return render ( request , 'criacao.html' , {'form': frm} )
